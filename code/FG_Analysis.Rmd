...
```{r}
count_tables <- list(bact_count, fungi_all_count, fungi_AMF_count)

div_test( , qvalue = 0, hierarchy = hierarchy_table)

q0 <- for (i in 1:length(count_tables)) {
  count_tables[[i]][["Avg"]]<-rowMeans(df.list[[i]][1:2])
}
```
