...
We have now created a taxmap object. The first line tells us that the ASVs are assigned to 3,718 unique taxa. The second line tells us how the taxa are related to one another in the tree. The original data can now be found in bact_obj$data$tax_data. Note that this tax_data table has a column called "taxon_id" which associates rows in the table to a taxa in the taxonomy.
...
```{r}
bact_count2 <- tibble::rownames_to_column(bact_count, "ASV")

bact_taxa$ASV <- as.character(bact_taxa$ASV)
bact_count2$ASV <- as.character(bact_count2$ASV)

bact_ASV_data <- left_join(bact_count2, bact_taxa,
                           by = c("ASV" = "ASV"))
```
 
It's worth knowing how many singletons we have in our bacterial and fungal datasets. We can use the zero_low_counts function in the metacoder package to convert any ASV counts less than some number to zero.
```{r}
```
